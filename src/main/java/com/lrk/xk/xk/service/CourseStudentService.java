...
import com.lrk.xk.xk.model.CourseStudent;
...
public class CourseStudentService extends DaoSupportImpl<CourseStudent> {
    ...
    public Message selectCourses(int userID, Integer[] ids) {
        ...
        if (session.createQuery("FROM CourseStudent WHERE sid = (:sid) and cid in (:cid)")
            .setParameter("sid", userID).setParameterList("cid", ids).list().size() > 0) {
    ...
    public Message unselectCourses(int userID, Integer[] ids) {
        ...
        Transaction tx = session.beginTransaction();
        int rows =
            session.createQuery("DELETE FROM CourseStudent WHERE sid = (:sid) and cid in (:cid)")
                .setParameter("sid", userID).setParameterList("cid", ids).executeUpdate();
        tx.commit();
    ...
    public Message recordGrade(int tid, int cid, int sid, double grade) {
        ...
        if (session.createQuery("FROM TeacherCourse WHERE tid = (:tid) and cid = (:cid)")
            .setParameter("tid", tid).setParameter("cid", cid).list().size() == 0)
            return new Message(-1, "您非此课程的授课教师，无法录入成绩");
        Transaction tx = session.beginTransaction();
        int row = session.createQuery(
            "UPDATE CourseStudent SET grade = (:grade) WHERE cid = (:cid) and sid = (:sid)")
            .setParameter("grade", grade).setParameter("cid", cid).setParameter("sid", sid)
            .executeUpdate();
        tx.commit();
    ...
    public Map<String, List> queryGrade(int sid) {
        ...
        if (cids.length == 0)
            ...
            courses =
                session.createQuery("FROM Course WHERE id in (:cid)").setParameterList("cid", cids)
                    .list();
